  ...
  useEffect(() => {
    const loadTopPlayers = async () => {
      try {
        setIsLoading(true);
        setLoadError(null);
        const data = await getTopPlayersByWins({ userId, startDate, endDate, currency });
        ...
        setLoadError('No se pudieron cargar los mejores ganadores.');
        setPlayers([]);
      } finally {
        setIsLoading(false);
      }
  ...
  const visiblePlayers = useMemo(() => {
    ...
    return players
      .map((row) => ({
        ...
        amount: getTopPlayerMetricAmount(row, 'wins', currency),
      }))
      .slice(0, maxItems);
    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <main className="flex-1 px-4 md:px-8 py-8">
        <div className="max-w-6xl mx-auto space-y-6">
          {/* Header */}
          <div>
            <h1 className="text-3xl font-bold text-primary mb-2">Top Winners</h1>
            <p className="text-muted-foreground">Los jugadores con mayor cantidad ganada</p>
          </div>
          ...
          <div className="flex gap-4 flex-col md:flex-row items-start md:items-center">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                ...
                <option value="20">20 Ganadores</option>
                <option value="50">50 Ganadores</option>
                <option value="100">100 Ganadores</option>
              </select>
            </div>
          </div>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-border bg-muted/50">
                      <th className="px-6 py-4 text-left text-sm font-semibold text-muted-foreground">TOTAL GANADO ({currency})</th>
                  ...
                  <tbody>
                    {visiblePlayers.map(({ row, amount }, idx) => (
                      <tr key={row?.id ?? row?.playerId ?? idx} className="border-b border-border hover:bg-muted/30 transition-colors">
                        <td className="px-6 py-4 text-sm font-semibold text-primary flex items-center gap-2">
                          <span>{formatPlayerAmountByCurrency(amount, currency)}</span>
